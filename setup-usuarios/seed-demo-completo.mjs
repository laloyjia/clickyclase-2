/**
 * seed-demo-completo.mjs — Click&Clase
 * ═══════════════════════════════════════════════════════════════
 * SEED DEMO PARA ENVIAR A COLEGIOS — 4 cursos, todos los perfiles.
 *
 * Liceo: "Colegio Demo"
 * Slug:  demo
 ...
 * Password: Demo123..
 *
 * USUARIOS (todos con password Demo123..):
 *   • rector@demo.cl              → Rector/a
 *   • director@demo.cl            → Director/a de Ciclo
 *   • admin@demo.cl               → Administrador/a (admin_colegio)
 *   • utp@demo.cl                 → Jefa UTP
 *   • pie.coord@demo.cl           → Encargada PIE
 *   • pie.edu1..3@demo.cl         → Educadoras Diferenciales
 *   • apoyo.enc@demo.cl           → Encargada Apoyo Psicosocial
 *   • psicologo@demo.cl           → Psicólogo
 *   • trabajador.social@demo.cl   → Trabajadora Social
 *   • psicopedagoga@demo.cl       → Psicopedagoga
 *   • fonoaudiologo@demo.cl       → Fonoaudiólogo
 *   • conviv.enc@demo.cl          → Encargado Convivencia
 *   • mediador@demo.cl            → Mediadora
 *   • inspector@demo.cl           → Inspector General
 *   • [nombre].[apellido]@demo.cl → 15+ Profesores (uno por asignatura, incl. jefes)
 *
 * CURSOS (4):
 *   1. 8°A       (jefe: sofia.herrera)
 *   2. 1° Medio A (jefe: pablo.contreras)
 *   3. 2° Medio A (jefe: sebastian.castillo)
 *   4. 3° Medio A · TP Electrónica (jefe: andres.reyes)
 *
 * Datos poblados en cada curso:
 *  ✅ ~30 estudiantes con RUT válido + apoderados
 *  ✅ Notas de todas las asignaturas
 *  ✅ Asistencia de últimos 2 meses
 *  ✅ Anotaciones (positivas y negativas)
 *  ✅ Alertas a apoderado
 *  ✅ Derivaciones a APS/PIE/Convivencia
 *  ✅ Fichas PIE con adecuaciones
 *  ✅ Intervenciones apoyo psicosocial
 *  ✅ Entrevistas convivencia
 *  ✅ Materiales + planificaciones + libro de clases
 *  ✅ Comunicados + eventos calendario
 *
 * Uso:
 *   cd setup-usuarios
 *   node seed-demo-completo.mjs
 *
...
async function crearFuncionarios() {
  ...
  const staff = [
    ...
    // ══ UTP ══
    { rol: 'utp',           nombre: 'Marcela Espinoza Vargas',          email: 'utp',       cargo: 'Jefa UTP' },
    ...
    { rol: 'aps_prof',      nombre: 'Ignacio Bravo Torres',             email: 'fonoaudiologo',     cargo: 'Fonoaudiólogo Escolar', profesion: 'Fonoaudiólogo' },
    // ══ CONVIVENCIA ESCOLAR ══
    { rol: 'amb_enc',       nombre: 'Mauricio Sepúlveda Rojas',         email: 'conviv.enc',cargo: 'Encargado Convivencia Escolar', profesion: 'Orientador' },
    { rol: 'amb_prof',      nombre: 'Valentina Morales Flores',         email: 'mediador',  cargo: 'Mediadora Escolar',           profesion: 'Mediadora Escolar' },
    { rol: 'amb_prof',      nombre: 'Rodrigo Silva Castro',             email: 'inspector', cargo: 'Inspector General',           profesion: 'Inspector' },
